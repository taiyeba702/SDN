...
class ProjectController(app_manager.RyuApp):
    ...
    def get_path (self, src, dst):#implementation of Floyd Warshall
            
        print("\nThe source", src,"to the destination", dst)    
        distance = defaultdict(lambda: defaultdict(lambda: 0))
        previous = defaultdict(lambda: defaultdict(lambda: 0))
        
        for i in self.switches: #Assigning nodes and edges Initially
            for j in self.switches: 
                distance[i][i] = 0
                distance[j][j] = 0    
                           
                if self.adjacency[i][j] != None:              
                     previous[i][j] = j
                     distance[i][j] = j-i
                else: 
                     distance[i][j] = float('Inf')                      
               
        for k in self.switches: #Finding Shortest Path
        ...
        if previous[src][dst] is None: #Append Paths
            return []
        path = [src]
        if src != dst:
            while path[-1] != dst:
                path.append(previous[path[-1]][dst])
    ...
    def generate_openflow_gid(self): #Generates Random Openflow group ID        
    ...
    def packet_in_handler(self, ev):
        ...
        # Filtering LLDP Packets and avoid their broadcast
    ...
    @set_ev_cls(event.EventSwitchEnter) #when a switch enters the network 
    ...
    @set_ev_cls(event.EventSwitchLeave, MAIN_DISPATCHER) #when a switch leaves the network
    ...
    def link_delete_handler(self, ev):
        s1 = ev.link.src
        s2 = ev.link.dst
        try: #handling exception when the switch is already deleted
